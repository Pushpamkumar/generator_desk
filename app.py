    ...
    if data_source == "Use Sample Data":
        try:
            ...
            if company_name_clean:
                matched = sample_data[sample_data['Company_clean'] == company_name_clean]
                if len(matched) > 0:
                    user_financials = matched.drop(columns=['Company_clean'])
                    st.success(f"✓ Sample data loaded for {matched['Company'].iloc[0]}")
                else:
                    st.info("Sample data found, but company name did not match exactly. Try 'Tata Motors' or one of the sample names.")
            else:
                st.info("Sample data available. Enter one of the sample company names above to load data.")

            if user_financials is not None and len(user_financials) > 0:
        ...
        if uploaded_file:
            try:
                ...
                missing = required_cols - set(user_financials.columns)
                if missing:
